...
- **AI Execution (KataGo):**
  ...
  - **Engine Configuration (Local vs Remote):** The app's architecture will abstract the engine interface, allowing users to configure the AI source:
    - *Desktop:* Uses the local bundled executable (offline).
    - *Mobile (Power User Mode):* Users can run a lightweight KataGo server script on their desktop PC, and configure the mobile app to connect to their desktop's local IP address (over local WiFi). This allows a powerful, free mobile app immediately without cloud costs.
    - *Mobile (Future):* Point the app to a managed cloud server.
